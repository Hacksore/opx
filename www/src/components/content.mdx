...
It will check [`packageManager`](https://nodejs.org/dist/latest/docs/api/all.html#all_packages_packagemanager) definition and use that to start your app. The default package manager is `npm` and the default script is `dev`. You can override the default script with `opx.defaultScript` in `package.json`.
...
> **Warning:** `--tty` disables 1Password's output masking. Secret injection still works, but any secret printed by the command will be visible in plaintext and may remain in terminal scrollback, recordings, or logs. Only use `--tty` with commands you trust not to print secrets.
